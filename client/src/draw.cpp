...
void drawShapePartColor(sf::RenderWindow* window, const sf::Vector2f& origin, const ShapePart& part, sf::Color color) {
    sf::CircleShape shape(part.getRadius());
...
{
		if(food.getFoodType()==FoodType::SIZE){
			drawShapePart(window, origin, food.getPart());
		}
		else if(food.getFoodType()==FoodType::SPEED){
			drawShapePartColor(window, origin, food.getPart(),sf::Color::Red);
		}
