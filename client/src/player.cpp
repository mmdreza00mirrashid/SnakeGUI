...
void Player::init() {
	...
	if (!buffer.loadFromFile("build/static/game-start.wav")) {
...
void Player::computeFoodsIntersection() {
	...
	for (it_food = getFoods().begin(); it_food != getFoods().end();) {
		if (_snake.checkFoodIntersection(*it_food)) {
			eat.play();
			Food currentFood = *it_food;
			if(currentFood.getFoodType()==FoodType::SIZE){
				_snake.addTail(1);
				
			}
			else if(currentFood.getFoodType()==FoodType::SPEED){
				isBoosted=true;
				std::cout<<"test";
			}
