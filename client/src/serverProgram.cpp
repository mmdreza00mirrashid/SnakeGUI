...
unsigned int serverProgram::_player_id_counter = 0;
...
void serverProgram::run () {
	...
	while (_is_running) {
		...
		cv_ready_compute.wait_for(lk_compute, std::chrono::milliseconds(18));

		std::this_thread::sleep_for(std::chrono::milliseconds(18));
	}
}

void serverProgram::initializeFoods() {
	for (size_t i = 0; i < FOOD_NUMBER - 1; i++) {
		Food food=Food(FoodPart::generateRandom(sf::Color::White));
		if(i%6==0){
			food.setFoodType(FoodType::SPEED);
			}
		else{
				food.setFoodType(FoodType::SIZE);
			}
		_foods.push_back(food);
	}
}

void serverProgram::initializeAIs() {
