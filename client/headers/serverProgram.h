...
class serverProgram {
	public:
		...
		static serverProgram& instance() {
			return serverprogram;
		}

        serverProgram(serverProgram const&) = delete;

        void operator=(serverProgram const&) = delete;
	...
	private:
		...
		serverProgram (int port): _server(port, &_socket_queue) {
